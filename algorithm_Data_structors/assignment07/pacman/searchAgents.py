...
class auto(SearchAgent):
  ...
  def findPathToClosestDot(self, gameState):
    ...
    if(food[startPosition[0]-1][startPosition[1]]):
      return [Directions.WEST]
    if(food[startPosition[0]+1][startPosition[1]]):
      return [Directions.EAST]
    if(food[startPosition[0]][startPosition[1]-1]):
      return [Directions.SOUTH]
    if(food[startPosition[0]][startPosition[1]+1]):
      return [Directions.NORTH]

    print "Error: auto pacman only works with a complete path to goal"
    sys.exit(0)
